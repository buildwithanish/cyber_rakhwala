...
const EvidenceBoard = () => {
  ...
  const handleCreateEvidence = async () => {
    ...
    const result = await createEvidence(newEvidence);
    if (result.success) {
  ...
  // Handle add tag
  const handleAddTag = async (evidenceId, tag) => {
    if (!tag.trim()) return;
    await addTag(evidenceId, tag.trim());
    setNewTag('');
  };

  // Handle remove tag
  const handleRemoveTag = async (evidenceId, tag) => {
    await removeTag(evidenceId, tag);
  };
