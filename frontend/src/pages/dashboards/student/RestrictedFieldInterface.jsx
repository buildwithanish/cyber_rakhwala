...
import {
  ...
  MessageCircle, Car, Wallet, Fingerprint, CreditCard, BarChart3, Network, Users,
...
const toolsData = [
  ...
  { id: 'image-forensics', name: 'Image Forensics', icon: Image, category: 'Forensics', cost: 22, cmd: 'img-meta' },
  ...
  { id: 'aadhaar-info', name: 'Aadhaar Info', icon: Fingerprint, category: 'Identity', cost: 20, cmd: 'aadhaar-lookup' },
  { id: 'pan-lookup', name: 'PAN Lookup', icon: CreditCard, category: 'Financial', cost: 18, cmd: 'pan-trace' },
  { id: 'cdr-analysis', name: 'CDR Analysis', icon: BarChart3, category: 'Telecom', cost: 25, cmd: 'cdr-analyze' },
  { id: 'ipdr-analysis', name: 'IPDR Analysis', icon: Network, category: 'Telecom', cost: 25, cmd: 'ipdr-analyze' },
  { id: 'person-location', name: 'Person Location', icon: Users, category: 'Records', cost: 20, cmd: 'person-lookup' },
];
...
const RestrictedFieldInterface = () => {
  ...
  const [terminalHistory, setTerminalHistory] = useState([
    { type: 'system', text: '◉ CYBER RAKHWALA HOLOGRAPHIC INTERFACE v3.0' },
  ...
  const toolComponents = {
    ...
    'aadhaar-info': AadhaarInfoTool,
    'pan-lookup': PANInfoTool,
    'cdr-analysis': CDRAnalysisTool,
    'ipdr-analysis': IPDRAnalysisTool,
    'person-location': PersonLocationTool,
  };
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        if (event.results[0].isFinal) {
          ...
          const command = transcript.toLowerCase()
            .replace('hey rakhwala', '')
            .replace('cyber rakhwala', '')
  ...
  const startVoiceRecognition = () => {
    if (recognitionRef.current && !isListening) {
      setIsListening(true);
      addTerminalLine('info', '🎤 Listening... Say "Hey Rakhwala" followed by your command');
  ...
  const exportData = (type) => {
    ...
    switch(type) {
      case 'commands':
        data = {
          type: 'cyber_rakhwala_saved_commands',
          version: '1.0',
      ...
      case 'history':
        data = {
          type: 'cyber_rakhwala_scan_history',
          version: '1.0',
          exportedAt: new Date().toISOString(),
  ...
  const handleFileImport = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.type === 'cyber_rakhwala_saved_commands' && Array.isArray(data.commands)) {
        ...
        } else if (data.type === 'cyber_rakhwala_session') {
          ...
          setImportError('Invalid file format. Please use a valid Cyber Rakhwala export file.');
        }
      } catch (err) {
  ...
  const exportToolResults = (toolId) => {
    ...
    const data = {
      type: 'cyber_rakhwala_tool_export',
      version: '1.0',
      exportedAt: new Date().toISOString(),
      tool: {
        id: tool.id,
  ...
  const handleToolFileImport = (event, toolId) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.type === 'cyber_rakhwala_tool_export' && data.tool?.id === toolId) {
    ...
    <div className={`h-screen bg-[#010408] text-gray-100 overflow-hidden relative ${getThemeClass()}`}>
      ...
      <div className="absolute top-0 left-0 right-0 z-[50]">
        <div className="flex items-center justify-between px-3 md:px-6 py-2 md:py-4">
          ...
          >
            <h1 
              className="text-xl sm:text-2xl md:text-4xl text-cyan-400 tracking-wider drop-shadow-[0_0_20px_rgba(6,182,212,0.6)]"
              style={{ fontFamily: 'Papyrus, fantasy, cursive' }}
            >
              CYBER RAKHWALA
            </h1>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-6">
                Import previously exported Cyber Rakhwala data files (.json)
              </p>
